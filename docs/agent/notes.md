...
- ADB bring-up has moved to `/opt/adb/hootswitch`; this repo no longer contains the ADB firmware modules or CDC2 interface. Use `/opt/adb/miscdocs/an591b.pdf` for timing/background when updating hootswitch.
...
- When `/dev/ttyACM*` numbers jump, use `udevadm info -n /dev/ttyACM2 | rg "ID_USB_INTERFACE_NUM"` to map the node to CDC0/1.
