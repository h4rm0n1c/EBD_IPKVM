...
- ATtiny85/trabular SPI framing defaults to 16-bit transfers with the command byte in the low byte and the response returned in the high byte of the next transfer; CS high resets the USI counter between bytes.
- SPI command byte placement can be flipped at build time via `ADB_SPI_CMD_IN_HIGH_BYTE` to accommodate alternate USI framing expectations (set to 1 for high-byte commands).
