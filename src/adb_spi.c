...
/*
 * Park SPI pins as plain GPIO inputs with no pulls.
...
uint8_t adb_spi_xfer(uint8_t cmd) {
    ...
    /*
     * 8-bit SPI frame. The returned MISO byte is the response to the
     * previous command; callers should issue a dummy transfer (0x00)
     * after commands that expect a response.
     */
    uint8_t tx8 = cmd;
    uint8_t rx8 = 0;
